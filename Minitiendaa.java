...
public class Minitiendaa {
    ...
    Producto[] resultado=new Producto[t];
    t=0;
    for(int i=0;i<cat.length;i++){
if(cat[i].getPrecio()<=limite){
            t++;
    ...
    public static int indiceMasCaro(Producto[] cat) {
        int mejor=0;
         for(int i=1;i<cat.length;i++){
            if(cat[i].getPrecio()>cat[mejor].getPrecio()){
i=mejor;
return mejor;
            }
         }
    ...
    // Pasos sugeridos:
    //   - Crea una copia de referencias: Producto[] res = Arrays.copyOf(cat, cat.length);
    ...
    public static Producto[] aplicarDescuento(Producto[] cat, double porcentaje) {
        Producto[] res=new Producto[cat.length];
        for(int i=0; i<cat.length; i++){
           double precioNuevo=getPrecio() *(1-porcentaje/100);
        }
    ...
    public static void main(String[] args) {
        ...
        if (iCaro >= 0)   System.out.println("Más caro: " + datos[iCaro]);
        if (iBarato >= 0) System.out.println("Más barato: " + datos[iBarato]);
